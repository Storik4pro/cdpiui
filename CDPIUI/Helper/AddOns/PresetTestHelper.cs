...
namespace CDPIUI.Helper.AddOns
{
    ...
    public class PresetTestHelper
    {
        ...
        {
            return Path.Combine(Directories.SettingsDirectory, "test_targets.txt");
        }
            ...
            CancellationToken ct)
        {
            ...
            try
            {
                ...
                foreach (ConfigItem preset in presets)
                {
                    ...
                    string args = ConfigurationService.GetStartupParametersByConfigItem(preset);
                    ...
                    await processManager.StartProcess(args);
                    await SafeDelay(StartWaitMs, ct);

                    if (!processManager.IsProcessRunning || processManager.IsErrorHappens)
                    {
        ...
        private void SaveResultsToFile(string componentId, PresetTestType testType, List<PresetTestResult> results, IProgress<List<TestLogSegment>> log)
        {
            try
            {
                string logsDir = Path.Combine(Core.Data.Directories.DataDirectory, "Logs", "test results");
